...
class FlaskTestCase(unittest.TestCase):
    ...
    def test_add_member_invalid(self):
        tester = app.test_client(self)
        response = tester.post(
            "/add_member", data=dict(name="a", email="a"), follow_redirects=True
        )
    ...
    def test_edit_member_invalid(self):
        tester = app.test_client(self)
        response = tester.post(
            "/edit_member/1", data=dict(name="a", email="a"), follow_redirects=True
        )
        self.assertIn(b"Field must be", response.data)
    ...
    def test_add_book_invalid(self):
        tester = app.test_client(self)
        response = tester.post(
            "/add_book", data=dict(id="1", title="a"), follow_redirects=True
        )
        self.assertIn(b"Field must be", response.data)
    ...
    def test_edit_book_invalid(self):
        tester = app.test_client(self)
        response = tester.post("/edit_book/1", data=dict(id="1"), follow_redirects=True)
        self.assertIn(b"Field must be", response.data)
    ...
    def test_import_books_invalid(self):
        tester = app.test_client(self)
        response = tester.post(
            "/import_books", data=dict(no_of_books="1"), follow_redirects=True
        )
    ...
    def test_issue_book_invalid(self):
        tester = app.test_client(self)
        response = tester.post(
            "/issue_book", data=dict(book_id="1", member_id="1"), follow_redirects=True
        )
        self.assertIn(b"Number must be", response.data)
