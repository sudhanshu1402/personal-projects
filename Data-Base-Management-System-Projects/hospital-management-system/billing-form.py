...
class Billing:
    def __init__(self, master):
        ...
        # ===============TITLE==========
        self.lblTitle = Label(
            self.frame, text="BILLING WINDOW", font="Helvetica 20 bold", bg="cadet blue"
        )
        ...
        self.lblpid = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.P_id
        )
        self.lblpid.grid(row=0, column=1)

        self.lbldid = Label(
            self.LoginFrame,
            text="DATE DISCHARGED(YYYY-MM-DD)",
            font="Helvetica 14 bold",
            bg="cadet blue",
            bd=22,
        )
        ...
        self.lbldid = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.dd
        )
        self.lbldid.grid(row=1, column=1)
        self.button2 = Button(
            self.LoginFrame,
            text="UPDATE DISCHARGE DATE",
            width=25,
            font="Helvetica 14 bold",
            bg="cadet blue",
        ...
        self.lbltreat = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.treat_1
        )
        self.lbltreat.grid(row=2, column=1)

        self.lblcode_t1 = Label(
            self.LoginFrame,
            text="TREATMENT CODE",
            font="Helvetica 14 bold",
            bg="cadet blue",
        ...
        self.lblcode_t1 = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.treat_2
        )
        ...
        self.lblap = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.cost_t
        )
        ...
        self.lblmed = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.med
        )
        ...
        self.med_t1 = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.med_q
        )
        ...
        self.lblapd = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.price
        )
    ...
    def UPDATE_DATE(self):
        ...
        b2 = self.dd.get()
        conn.execute(
            "UPDATE ROOM SET DATE_DISCHARGED=? where PATIENT_ID=?",
            (
                b2,
                b1,
            ),
        ...
        tkinter.messagebox.showinfo(
            "HOSPITAL DATABASE SYSTEM", "DISCHARGE DATE UPDATED"
        )
    ...
    def UPDATE_DATA(self):
        ...
        b8 = self.price.get()
        p = list(
            conn.execute("Select * from TREATMENT where TREATMENT.PATIENT_ID=?", (b1,))
        )
        if len(p) != 0:
            tkinter.messagebox.showerror(
                "HOSPITAL DATABSE SYSTEM", "PATIENT ID IS ALREADY REGISTERED"
            )
        else:
            conn.execute(
            ...
            conn.commit()
            tkinter.messagebox.showinfo(
                "HOSPITAL DATABASE SYSTEM", "BILLING DATA SAVED"
            )
