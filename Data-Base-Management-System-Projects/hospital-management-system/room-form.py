...
class Room:
    ...
    def __init__(self, master):
        ...
        self.room_t1 = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.room_t
        )
        ...
        self.room_no1 = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.room_no
        )
        ...
        self.lblrate = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.rate
        )
        ...
        self.lblda = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.da
        )
    ...
    def INSERT_ROOM(self):
        ...
        if x != 0:
            tkinter.messagebox.showerror(
                "HOSPITAL DATABASE SYSTEM", "ROOM_NO IS CURRENTLY OCCUPIED"
            )
        else:
            conn.execute(
    ...
    def UPDATE_ROOM(self):
        ...
        p = list(
            conn.execute(
                "Select * from ROOM where PATIENT_ID=? AND ROOM_NO=?",
                (
                    r1,
                    r3,
                ),
        ...
        if len(p) != 0:
            tkinter.messagebox.showerror(
                "HOSPITAL DATABSE SYSTEM", "PATIENT IS NOT ALLOCATED A ROOM"
            )
            ...
            tkinter.messagebox.showinfo(
                "HOSPITAL DATABSE SYSTEM", "ROOM DETAILS UPDATED"
            )
            conn.commit()
...
class S_Room:
    def __init__(self, master):
        ...
        self.lblpatid.grid(row=0, column=0)
        self.lblpatid = Entry(
            self.LoginFrame, font="Helvetica 14 bold", bd=2, textvariable=self.Pr_id
        )
        self.lblpatid.grid(row=0, column=1)
    ...
    def ROOM_DISPLAY(self):
        ...
        if len(p) == 0:
            tkinter.messagebox.showerror(
                "HOSPITAL DATABASE SYSTEM", "PATIENT NOT ALLOCATED ROOM"
            )
        else:
            t = c1.execute(
                "SELECT PATIENT_ID,NAME,ROOM_NO,ROOM_TYPE FROM ROOM NATURAL JOIN PATIENT where PATIENT_ID=?",
                (pat_rm,),
            )
            for i in t:
