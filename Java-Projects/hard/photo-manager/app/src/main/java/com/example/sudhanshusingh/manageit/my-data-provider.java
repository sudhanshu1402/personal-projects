...
public class MyDataProvider extends ContentProvider {
  ...
  public Cursor query(
    ...
    if (match == CATEGORY)
      return sqLiteDatabase.query(MyDatabaseHelper.CATEGORY_TABLE_NAME,
          MyDatabaseHelper.ALL_CATEGORY_TABLE_COLUMNS, selection, selectionArgs, null, null,
          MyDatabaseHelper.CATEGORY_NAME + " COLLATE NOCASE ASC");

    else if (match == TAGS) {
      return sqLiteDatabase.query(MyDatabaseHelper.TABLE_NAME, MyDatabaseHelper.ALL_COLUMNS,
          selection, selectionArgs, null, null, MyDatabaseHelper.TAG_NAME + " COLLATE NOCASE ASC");
    }
  ...
  public int delete(Uri uri, String selection, String[] selectionArgs) {
    ...
    if (match == CATEGORY) {
      int delete =
          sqLiteDatabase.delete(MyDatabaseHelper.CATEGORY_TABLE_NAME, selection, selectionArgs);
      context.getContentResolver().notifyChange(CATEGORY_CONTENT_URI, null);
      return delete;
    } else if (match == TAGS) {
  ...
  public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
    int match = uriMatcher.match(uri);
    if (match == CATEGORY) {
      int update = sqLiteDatabase.update(
          MyDatabaseHelper.CATEGORY_TABLE_NAME, values, selection, selectionArgs);
      context.getContentResolver().notifyChange(CATEGORY_CONTENT_URI, null);
      return update;
    } else if (match == TAGS) {
      int update =
          sqLiteDatabase.update(MyDatabaseHelper.TABLE_NAME, values, selection, selectionArgs);
